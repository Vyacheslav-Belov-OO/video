...
if ( !function_exists('hotlock_need_page_title') ) {
	function hotlock_need_page_title() {
		return !is_front_page() && apply_filters('hotlock_filter_need_page_title', true);
	}
}
...
if ( !function_exists('hotlock_show_layout') ) {
	function hotlock_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('hotlock_get_logo_image') ) {
	function hotlock_get_logo_image($type='') {
		$logo_image = '';
		if (hotlock_get_retina_multiplier(2) > 1)
			$logo_image = hotlock_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = hotlock_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('hotlock_get_header_video') ) {
	function hotlock_get_header_video() {
		$video = '';
		if (apply_filters('hotlock_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (hotlock_check_theme_option('header_video')) {
				$video = hotlock_get_theme_option('header_video');
...
if ( !function_exists('hotlock_show_post_meta') ) {
	function hotlock_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			if ( !empty($args['categories'])) {
				$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('hotlock_filter_get_post_categories', '');
				if (!empty($cats)) {
					?>
					<span class="post_meta_item post_categories"><?php hotlock_show_layout($cats); ?></span>
					<?php
				}
			}
			// Post date
			if ( !empty($args['date'])) {
				$dt = apply_filters('hotlock_filter_get_post_date', hotlock_get_date());
				if (!empty($dt)) {
					?>
			...
			// Post counters
			hotlock_show_layout(hotlock_get_post_counters($args['counters']));

			// Socials share
			if ( !empty($args['share']) ) {
				hotlock_show_share_links(array(
						'type' => 'drop',
						'caption' => esc_html__('Share', 'hotlock'),
			...
			// Edit page link
			if ( !empty($args['edit']) ) {
				edit_post_link( esc_html__( 'Edit', 'hotlock' ), '<span class="post_meta_item post_edit icon-pencil">', '</span>' );
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('hotlock_show_post_featured') ) {
	function hotlock_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => hotlock_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : hotlock_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		...
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				...
				$image = hotlock_get_post_image();
				if (!empty($image)) 
					$image = hotlock_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = hotlock_get_no_image();
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . hotlock_add_inline_style('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="//schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$hotlock_attr = hotlock_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($hotlock_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($hotlock_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb || !empty($args['show_no_image']) || (!$args['thumb_only'] && in_array($post_format, array('gallery', 'image', 'audio', 'video')))) {
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						hotlock_hovers_add_icons($args['hover']);
					}
					...
					if ( ($output = hotlock_build_slider_layout(array('thumb_size' => $thumb_size, 'controls' => 'yes', 'pagination' => 'yes'))) != '' )
						hotlock_show_layout($output);
					...
					if (!empty($image)) {
						...
						<div class="mask"></div>
						<?php 
						hotlock_hovers_add_icons($args['hover'], array('image' => $image));
					}
				} else if (!empty($args['show_no_image']) && ($image=hotlock_get_no_image())!='') {
					?>
					<img src="<?php echo esc_url($image); ?>" alt="<?php the_title_attribute(); ?>">
					<div class="mask"></div>
					<?php 
				...
				if ($post_format == 'video' && !$args['thumb_only']) {
					//NEW
					$post_content = hotlock_get_post_content();
					$video = hotlock_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = hotlock_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = hotlock_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = hotlock_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = hotlock_make_video_autoplay( $video );
								?>
				...
				if ($post_format == 'audio' && !$args['thumb_only']) {
					$audio = hotlock_get_post_audio('', false);
					if (empty($audio))
						$audio = hotlock_get_post_iframe('', false);
					if (!empty($audio)) {
						//Show metadata (for the future version)
						if (false && function_exists('wp_read_audio_metadata')) {
							$src = hotlock_get_post_audio($audio);
						...
						?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
							...
							$media_author = hotlock_get_theme_option('media_author', '', false, get_the_ID());
							$media_title = hotlock_get_theme_option('media_title', '', false, get_the_ID());
							if ( !empty($media_author) && !hotlock_is_inherit($media_author) ) {
								?><div class="post_audio_author"><?php hotlock_show_layout($media_author); ?></div><?php
							}
							if ( !empty($media_title) && !hotlock_is_inherit($media_title) ) {
								?><h5 class="post_audio_title"><?php hotlock_show_layout($media_title); ?></h5><?php
							}
							// Display audio
							hotlock_show_layout($audio); 
						?></div><?php
					}
				}
				
				// Put optional info block over the thumb
				hotlock_show_layout($post_info);
				?></div><?php
			}
		}
	}
...
if ( !function_exists('hotlock_get_no_image') ) {
	function hotlock_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = hotlock_get_theme_option( 'no_image' );
			// Remove false in next line if you want display theme-specific image in the posts without featured image
			if (false && empty($img)) $img = hotlock_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('hotlock_add_bg_in_post_nav') ) {
	function hotlock_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = hotlock_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), hotlock_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
				...
				$css .= '.post-navigation .nav-previous a .nav-arrow {display:none;} .post-navigation .nav-previous a .nav-arrow { background-color: rgba(128,128,128,0.05); border-color:rgba(128,128,128,0.1); }';
		}
	
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), hotlock_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
				...
				$css .= '.post-navigation .nav-next a .nav-arrow {display:none;} .post-navigation .nav-next a .nav-arrow { background-color: rgba(128,128,128,0.05); border-color:rgba(128,128,128,0.1); }';
		}
	
		wp_add_inline_style( 'hotlock-main', $css );
	}
}

// Show related posts
if ( !function_exists('hotlock_show_related_posts') ) {
	function hotlock_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h3 class="section title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'hotlock');
				?></h3>
				<div class="columns_wrap posts_container">
					<?php
					while ( $query->have_posts() ) { $query->the_post();
						?><div class="column-2_<?php echo intval(max(2, $args['posts_per_page'])); ?>"><?php
...
if ( !function_exists('hotlock_show_portfolio_posts') ) {
	function hotlock_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? hotlock_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = hotlock_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = hotlock_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the hotlock_ajax_get_posts action
if ( !function_exists( 'hotlock_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_hotlock_ajax_get_posts',			'hotlock_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_hotlock_ajax_get_posts',	'hotlock_ajax_get_posts_callback');
	function hotlock_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			hotlock_storage_set('blog_mode', 'blog');
			hotlock_storage_set('options_meta', get_post_meta($id, 'hotlock_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => hotlock_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'hotlock');
		}
...
// Show pagination
if ( !function_exists('hotlock_show_pagination') ) {
	function hotlock_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = hotlock_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'hotlock' ),
				'next_text' => esc_html__( '>', 'hotlock' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'hotlock' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'hotlock'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'hotlock') : esc_html__('Newest posts', 'hotlock') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'hotlock') : esc_html__('Older posts', 'hotlock'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'hotlock_single_comments_field' ) ) {
    function hotlock_single_comments_field( $args ) {
