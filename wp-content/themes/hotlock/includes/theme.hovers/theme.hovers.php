...
if (!function_exists('hotlock_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'hotlock_hovers_theme_setup3', 3 );
	function hotlock_hovers_theme_setup3() {
		// Add 'Buttons hover' option
		hotlock_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'hotlock'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'hotlock') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'hotlock'),
					'slide_left'	=> esc_html__('Slide from Left',	'hotlock'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'hotlock'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'hotlock') ),
				"std" => 'dots',
				"options" => array(
					'dots'	=> esc_html__('Dots',	'hotlock'),
					'icon'	=> esc_html__('Icon',	'hotlock'),
					'icons'	=> esc_html__('Icons',	'hotlock'),
					'zoom'	=> esc_html__('Zoom',	'hotlock'),
					'fade'	=> esc_html__('Fade',	'hotlock'),
					'slide'	=> esc_html__('Slide',	'hotlock'),
					'pull'	=> esc_html__('Pull',	'hotlock'),
					'border'=> esc_html__('Border',	'hotlock')
				),
				"type" => "hidden" //select
			) )
		);
...
if (!function_exists('hotlock_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'hotlock_hovers_theme_setup9', 9 );
	function hotlock_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'hotlock_hovers_frontend_scripts', 1010 );
		add_filter( 'hotlock_filter_localize_script','hotlock_hovers_localize_script' );
		add_filter( 'hotlock_filter_merge_scripts',	'hotlock_hovers_merge_scripts' );
		add_filter( 'hotlock_filter_merge_styles',	'hotlock_hovers_merge_styles' );
		add_filter( 'hotlock_filter_get_css', 		'hotlock_hovers_get_css', 10, 4 );
	}
}

// Enqueue hover styles and scripts
if ( !function_exists( 'hotlock_hovers_frontend_scripts' ) ) {
	function hotlock_hovers_frontend_scripts() {
		if ( hotlock_is_on(hotlock_get_theme_option('debug_mode')) && file_exists(hotlock_get_file_dir('includes/theme.hovers/theme.hovers.js')) )
			wp_enqueue_script( 'hotlock-hovers', hotlock_get_file_url('includes/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( hotlock_is_on(hotlock_get_theme_option('debug_mode')) && file_exists(hotlock_get_file_dir('includes/theme.hovers/theme.hovers.css')) )
			wp_enqueue_style( 'hotlock-hovers',  hotlock_get_file_url('includes/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('hotlock_hovers_merge_scripts')) {
	function hotlock_hovers_merge_scripts($list) {
...
if (!function_exists('hotlock_hovers_merge_styles')) {
	function hotlock_hovers_merge_styles($list) {
...
if (!function_exists('hotlock_hovers_localize_script')) {
	function hotlock_hovers_localize_script($arr) {
		$arr['button_hover'] = hotlock_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('hotlock_hovers_add_icons') ) {
	function hotlock_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '"
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'hotlock') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'hotlock');
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'hotlock_hovers_get_css' ) ) {
	function hotlock_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
.scheme_dark .sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['alter_dark']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important;}
.scheme_dark .button.sc_button_hover_slide_left:hover { color: {$colors['bg_color']} !important;}

.more-link.sc_button_hover_slide_left:hover { color: {$colors['text_hover']} !important; }
...
.sc_button_hover_style_inverse.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
...
.sc_button_hover_style_hover.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
...
.sc_button_hover_style_alter.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_link']} !important; }
.sc_button_hover_style_alter.sc_button_hover_slide_right {		background: linear-gradient(to left,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['alter_link']} !important; }
.sc_button_hover_style_alter.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['alter_link']} !important; }
.sc_button_hover_style_alter.sc_button_hover_slide_bottom {		background: linear-gradient(to top,		{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right top / 100% 210% {$colors['alter_link']} !important; }

.sc_button_hover_style_alterbd.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['bd_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_bd_color']} !important; }
...
.sc_services_default .sc_services_item:hover .sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['bg_color']} 50%) no-repeat scroll right bottom / 210% 100% !important; color: {$colors['text_dark']}!important}
.sc_services_default .sc_services_item .sc_button_hover_slide_left:hover {	background-position: left bottom !important; color: {$colors['text_dark']}!important}

header .sc_button_hover_slide_left {
	background: linear-gradient(to right,	{$colors['text_dark']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 210% 100% !important;
	color: 	{$colors['text_dark']}!important
}
header .sc_button_hover_slide_left:hover {
