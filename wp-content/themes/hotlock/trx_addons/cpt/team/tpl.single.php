		...
		<section class="team_member_header">	
			...
			if ( has_post_thumbnail() ) {
				?><div class="team_member_featured">
					<div class="team_member_avatar">
						<?php
						the_post_thumbnail( trx_addons_get_thumb_size('team_avatar'), array(
									'alt' => get_the_title(),
									'itemprop' => 'image'
									)
								);
			...
			<div class="team_member_description">
				...
				<div class="team_member_details">
					<?php
					$override_options = isset($TRX_ADDONS_STORAGE['override_options_'.get_post_type()])
										? apply_filters('trx_addons_filter_override_options_fields', $TRX_ADDONS_STORAGE['override_options_'.get_post_type()], get_post_type())
										: array();
					foreach ($override_options as $k=>$v) {
						if (!empty($v['details']) && !empty($meta[$k])) {
							?><div class="team_member_details_<?php echo esc_attr($k); ?>"><span class="team_member_details_label"><?php echo esc_html($v['title']); ?>: </span><span class="team_member_details_value"><?php
                                if ($k == 'email'){
                                    echo '<a href="mailto:'.antispambot($meta[$k]).'">'.esc_html($meta[$k]).'</a>';?></span></div><?php
                                }else {
                                    echo esc_html($meta[$k]); ?></span></div><?php
                                }
						}
					}
					?>
				...
				if (!empty($meta['brief_info'])) {
					...
					<div class="team_member_brief_info">
						<h5 class="team_member_brief_info_title"><?php esc_attr_e('Brief info', 'hotlock'); ?></h5>
