...
function hotlock_init_actions() {
	...
	if (windowWidth > 768) {
		jQuery('.woocommerce ul.products li .woocommerce-loop-product__title,.woocommerce-page ul.products li .woocommerce-loop-product__title').each(function(){
			if (jQuery(this).height() > titleHeight) titleHeight = jQuery(this).height();
		});

		jQuery('.woocommerce ul.products li .woocommerce-loop-product__title,.woocommerce-page ul.products li .woocommerce-loop-product__title').css({
			'height' : titleHeight,
		});
	}
}
		...
		if (window.mejs) {
			...
			cont.find('audio:not(.inited),video:not(.inited)').each(function() {
				if (jQuery(this).parents('.mejs-mediaelement').length == 0
					...
					&& (HOTLOCK_STORAGE['init_all_mediaelements'] || (!jQuery(this).hasClass('wp-audio-shortcode')
						&& !jQuery(this).hasClass('wp-video-shortcode') && !jQuery(this).parent().hasClass('wp-playlist')))) {
