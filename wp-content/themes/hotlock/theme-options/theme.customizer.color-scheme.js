/* global hotlock_color_schemes, hotlock_dependencies, Color */
...
( function( api ) {
	...
	for (var i in hotlock_color_schemes) {
		cssTemplate[i] = wp.template( 'hotlock-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'hotlock-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {
		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+hotlock_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(hotlock_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', hotlock_customizer_update_load_fonts);		
		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', hotlock_customizer_actions);
		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			hotlock_customizer_check_dependencies(jQuery(this));
		});
		// Refresh color controls
		hotlock_customizer_change_color_scheme(api('color_scheme_editor')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		hotlock_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		hotlock_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function hotlock_customizer_check_dependencies(cont) {
		"use strict";
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in hotlock_dependencies) {
				if (fld == id) {
					depend = hotlock_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('hotlock_options_no_use');
				} else {
					jQuery(this).hide().addClass('hotlock_options_no_use');
				}
			}
		});
	}
	...
	function hotlock_customizer_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme_editor') {
			hotlock_customizer_change_color_scheme(val);

		} else if (updateCSS) {
			// Check colors in scheme
			var simple = api('color_settings')()=='simple';
			for (opt in hotlock_color_schemes['default'].colors) {
				if (opt == id) {
					// Store new value in the color table
					hotlock_customizer_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							hotlock_customizer_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							hotlock_customizer_update_color_scheme('alter_hover', val);
						}
					}
					processed = true;
					break;
			...
			// Check theme fonts
			if (!processed) {
				for (opt in hotlock_theme_fonts) {
					for (rule in hotlock_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							hotlock_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) hotlock_customizer_update_css();
		}
	...
	// Actions buttons
	function hotlock_customizer_actions(e) {
		"use strict";
	...
	// Store new value in the color table
	function hotlock_customizer_update_color_scheme(opt, value) {
		"use strict";
		hotlock_color_schemes[api('color_scheme_editor')()].colors[opt] = value;
		api('scheme_storage').set(hotlock_serialize(hotlock_color_schemes))
	}
	
	// Store new value in the theme fonts
	function hotlock_customizer_update_theme_fonts(opt, rule, value) {
		"use strict";
		hotlock_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function hotlock_customizer_update_load_fonts() {
		"use strict";
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=hotlock_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in hotlock_theme_fonts) {
	...
	function hotlock_customizer_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in hotlock_color_schemes[value].colors) {
			if (api(opt) == undefined) continue;
			api( opt ).set( hotlock_color_schemes[value].colors[opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', hotlock_color_schemes[value].colors[opt] )
				.wpColorPicker( 'defaultColor', hotlock_color_schemes[value].colors[opt] );
		}
		updateCSS = true;
		hotlock_customizer_update_css();
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function hotlock_customizer_update_css() {
		"use strict";
		...
		var fonts = hotlock_customizer_add_theme_fonts(hotlock_theme_fonts);

		// Make styles and add into css
		css += hotlock_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in hotlock_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in hotlock_color_schemes[scheme].colors) {
				colors[i] = hotlock_color_schemes[scheme].colors[i];
			}
			
			// Make theme specific colors and tints
			colors = hotlock_customizer_add_theme_colors(colors);
	...
	function hotlock_customizer_add_theme_colors(colors) {
		"use strict";
		...
		colors.text_link_blend = hotlock_hsb2hex(hotlock_hex2hsb( colors.text_link, 2, -5, 5 ));
		colors.alter_link_blend = hotlock_hsb2hex(hotlock_hex2hsb( colors.alter_link, 2, -5, 5 ));
		return colors;
	}

	// Add custom theme fonts rules
	// Attention! Don't forget setup custom theme fonts rules also in the theme.styles.php
	function hotlock_customizer_add_theme_fonts(fonts) {
		"use strict";
		var rez = [];
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + hotlock_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + hotlock_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + hotlock_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function hotlock_customizer_prepare_css_value(val) {
		"use strict";
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
	...
	function hotlock_customizer_prepare_html_value(val) {
		"use strict";
