...
if (!function_exists('hotlock_customizer_add_theme_colors')) {
	function hotlock_customizer_add_theme_colors($colors) {
		if (substr($colors['text'], 0, 1) == '#') {
			$colors['bg_color_0']  = hotlock_hex2rgba( $colors['bg_color'], 0 );
			$colors['bg_color_02']  = hotlock_hex2rgba( $colors['bg_color'], 0.2 );
			$colors['bg_color_07']  = hotlock_hex2rgba( $colors['bg_color'], 0.7 );
			$colors['bg_color_08']  = hotlock_hex2rgba( $colors['bg_color'], 0.8 );
			$colors['bg_color_09']  = hotlock_hex2rgba( $colors['bg_color'], 0.9 );
			$colors['alter_bg_color_07']  = hotlock_hex2rgba( $colors['alter_bg_color'], 0.7 );
			$colors['alter_bg_color_04']  = hotlock_hex2rgba( $colors['alter_bg_color'], 0.4 );
			$colors['alter_bg_color_02']  = hotlock_hex2rgba( $colors['alter_bg_color'], 0.2 );
			$colors['alter_bd_color_02']  = hotlock_hex2rgba( $colors['alter_bd_color'], 0.2 );
			$colors['text_dark_07']  = hotlock_hex2rgba( $colors['text_dark'], 0.7 );
			$colors['text_link_02']  = hotlock_hex2rgba( $colors['text_link'], 0.2 );
			$colors['text_link_07']  = hotlock_hex2rgba( $colors['text_link'], 0.7 );
...
if (!function_exists('hotlock_customizer_add_theme_fonts')) {
	function hotlock_customizer_add_theme_fonts($fonts) {
		$rez = array();
		foreach ($fonts as $tag => $font) {
			if (substr($font['font-family'], 0, 2) != '{{') {
				$rez[$tag.'_font-family'] 		= !empty($font['font-family']) && !hotlock_is_inherit($font['font-family'])
														? 'font-family:' . trim($font['font-family']) . ';'
														: '';
				$rez[$tag.'_font-size'] 		= !empty($font['font-size']) && !hotlock_is_inherit($font['font-size'])
														? 'font-size:' . hotlock_prepare_css_value($font['font-size']) . ";"
														: '';
				$rez[$tag.'_line-height'] 		= !empty($font['line-height']) && !hotlock_is_inherit($font['line-height'])
														? 'line-height:' . trim($font['line-height']) . ";"
														: '';
				$rez[$tag.'_font-weight'] 		= !empty($font['font-weight']) && !hotlock_is_inherit($font['font-weight'])
														? 'font-weight:' . trim($font['font-weight']) . ";"
														: '';
				$rez[$tag.'_font-style'] 		= !empty($font['font-style']) && !hotlock_is_inherit($font['font-style'])
														? 'font-style:' . trim($font['font-style']) . ";"
														: '';
				$rez[$tag.'_text-decoration'] 	= !empty($font['text-decoration']) && !hotlock_is_inherit($font['text-decoration'])
														? 'text-decoration:' . trim($font['text-decoration']) . ";"
														: '';
				$rez[$tag.'_text-transform'] 	= !empty($font['text-transform']) && !hotlock_is_inherit($font['text-transform'])
														? 'text-transform:' . trim($font['text-transform']) . ";"
														: '';
				$rez[$tag.'_letter-spacing'] 	= !empty($font['letter-spacing']) && !hotlock_is_inherit($font['letter-spacing'])
														? 'letter-spacing:' . trim($font['letter-spacing']) . ";"
														: '';
				$rez[$tag.'_margin-top'] 		= !empty($font['margin-top']) && !hotlock_is_inherit($font['margin-top'])
														? 'margin-top:' . hotlock_prepare_css_value($font['margin-top']) . ";"
														: '';
				$rez[$tag.'_margin-bottom'] 	= !empty($font['margin-bottom']) && !hotlock_is_inherit($font['margin-bottom'])
														? 'margin-bottom:' . hotlock_prepare_css_value($font['margin-bottom']) . ";"
														: '';
			} else {
...
if (!function_exists('hotlock_customizer_get_css')) {

	function hotlock_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = hotlock_customizer_add_theme_fonts($fonts);
...
CSS;
			$rez = apply_filters('hotlock_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
		}

		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(hotlock_get_list_schemes()) : array($only_scheme);

			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = hotlock_get_scheme_colors($scheme);
...
.sticky.post_item .post_categories a{
	background-color: {$colors['alter_link']};
	color: {$colors['text_dark']};
}
.sticky.post_item .post_categories a:hover{
	background-color: {$colors['text_dark']};
	color: {$colors['alter_link']};
}
...
.post_layout_classic_2 .post_meta .post_categories a{
	color: {$colors['text_dark']};
}
.post_layout_classic_2 .post_meta .post_categories a:hover{
	color: {$colors['alter_link']};
}
...
/* Submenu */
.sc_layouts_menu_nav.menu_footer_nav  > li ul {
...
.hotlock_tabs .hotlock_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
.hotlock_tabs .hotlock_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.sidebar.scheme_dark input[type="submit"]{
	color: {$colors['text_dark']};
}
.sidebar.scheme_dark input[type="submit"]:hover{
	color: {$colors['text_link']};
}
...
CSS;
					...
					$rez = apply_filters('hotlock_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'hotlock_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>
