...
if (!function_exists('hotlock_trx_addons_theme_setup1')) {
	add_action( 'after_setup_theme', 'hotlock_trx_addons_theme_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'hotlock_trx_addons_theme_setup1', 8 );
	function hotlock_trx_addons_theme_setup1() {
		if (hotlock_exists_trx_addons()) {
			add_filter( 'hotlock_filter_list_posts_types',	'hotlock_trx_addons_list_post_types');
			add_filter( 'hotlock_filter_list_header_styles','hotlock_trx_addons_list_header_styles');
			add_filter( 'hotlock_filter_list_footer_styles','hotlock_trx_addons_list_footer_styles');
		}
	}
}
...
if (!function_exists('hotlock_trx_addons_theme_setup9')) {
	add_action( 'after_setup_theme', 'hotlock_trx_addons_theme_setup9', 9 );
	function hotlock_trx_addons_theme_setup9() {
		if (hotlock_exists_trx_addons()) {
			add_filter( 'trx_addons_sc_atts',							'hotlock_trx_addons_sc_atts', 10, 2);
			add_filter( 'trx_addons_sc_output',							'hotlock_trx_addons_sc_output', 10, 4);
			add_filter( 'trx_addons_filter_featured_image',				'hotlock_trx_addons_featured_image', 10, 2);
			add_filter( 'trx_addons_filter_no_image',					'hotlock_trx_addons_no_image' );
			add_filter( 'trx_addons_filter_get_list_icons',				'hotlock_trx_addons_get_list_icons', 10, 2 );
			add_filter( 'trx_addons_filter_slider_title',				'hotlock_trx_addons_slider_title', 10, 2 );
			add_filter( 'trx_addons_filter_meta_box_fields',			'hotlock_trx_addons_override_options_fields', 10, 2);
			add_action( 'wp_enqueue_scripts', 							'hotlock_trx_addons_frontend_scripts', 1100 );
			add_filter( 'hotlock_filter_query_sort_order',	 			'hotlock_trx_addons_query_sort_order', 10, 3);
			add_filter( 'hotlock_filter_merge_scripts',					'hotlock_trx_addons_merge_scripts');
			add_filter( 'hotlock_filter_get_css',						'hotlock_trx_addons_get_css', 10, 4);
			add_filter( 'hotlock_filter_prepare_css',					'hotlock_trx_addons_prepare_css', 10, 2);
			add_filter( 'hotlock_filter_prepare_js',					'hotlock_trx_addons_prepare_js', 10, 2);
			add_filter( 'hotlock_filter_get_post_categories',		 	'hotlock_trx_addons_get_post_categories');
			add_filter( 'hotlock_filter_get_post_date',		 			'hotlock_trx_addons_get_post_date');
			add_filter( 'trx_addons_filter_get_post_date',		 		'hotlock_trx_addons_get_post_date_wrap');
			add_filter( 'hotlock_filter_post_type_taxonomy',			'hotlock_trx_addons_post_type_taxonomy', 10, 2 );
			if (is_admin()) {
				add_filter( 'hotlock_filter_allow_override_options', 	'hotlock_trx_addons_allow_override_options', 10, 2);
				add_filter( 'hotlock_filter_allow_theme_icons', 		'hotlock_trx_addons_allow_theme_icons', 10, 2);
				add_filter( 'trx_addons_sc_map',						'hotlock_trx_addons_sc_map', 10, 2);
			} else {
				add_filter( 'trx_addons_filter_theme_logo',				'hotlock_trx_addons_theme_logo');
				add_filter( 'trx_addons_filter_post_meta',				'hotlock_trx_addons_post_meta', 10, 2);
				add_filter( 'hotlock_filter_get_mobile_menu',			'hotlock_trx_addons_get_mobile_menu');
				add_filter( 'hotlock_filter_detect_blog_mode',			'hotlock_trx_addons_detect_blog_mode' );
				add_filter( 'hotlock_filter_get_blog_title', 			'hotlock_trx_addons_get_blog_title');
				add_action( 'hotlock_action_login',						'hotlock_trx_addons_action_login', 10, 2);
				add_action( 'hotlock_action_search',					'hotlock_trx_addons_action_search', 10, 3);
				add_action( 'hotlock_action_breadcrumbs',				'hotlock_trx_addons_action_breadcrumbs');
				add_action( 'hotlock_action_show_layout',				'hotlock_trx_addons_action_show_layout', 10, 1);
			}
		}

		// Add this filter any time: if plugin exists - load plugin's styles, if not exists - load layouts.css instead plugin's styles
		add_filter( 'hotlock_filter_merge_styles',						'hotlock_trx_addons_merge_styles');

		if (is_admin()) {
			add_filter( 'hotlock_filter_tgmpa_required_plugins',		'hotlock_trx_addons_tgmpa_required_plugins' );
			add_action( 'admin_enqueue_scripts', 						'hotlock_trx_addons_editor_load_scripts_admin');
		}
	}
}

// Filter to add in the required plugins list
if ( !function_exists( 'hotlock_trx_addons_tgmpa_required_plugins' ) ) {
	function hotlock_trx_addons_tgmpa_required_plugins($list=array()) {
		if (in_array('trx_addons', (array)hotlock_storage_get('required_plugins'))) {
			$path = hotlock_get_file_dir('plugins/trx_addons/trx_addons.zip');
			$list[] = array(
					'name' 		=> esc_html__('ThemeREX Addons', 'hotlock'),
					'slug' 		=> 'trx_addons',
					'version'	=> '1.6.18.3',
...
if ( !function_exists( 'hotlock_exists_trx_addons' ) ) {
	function hotlock_exists_trx_addons() {
		return defined('TRX_ADDONS_VERSION');
	}
}

// Return true if team is supported
if ( !function_exists( 'hotlock_exists_team' ) ) {
	function hotlock_exists_team() {
		return defined('TRX_ADDONS_CPT_TEAM_PT');
	}
}

// Return true if services is supported
if ( !function_exists( 'hotlock_exists_services' ) ) {
	function hotlock_exists_services() {
		return defined('TRX_ADDONS_CPT_SERVICES_PT');
	}
}

// Return true if portfolio is supported
if ( !function_exists( 'hotlock_exists_portfolio' ) ) {
	function hotlock_exists_portfolio() {
		return defined('TRX_ADDONS_CPT_PORTFOLIO_PT');
	}
}

// Return true if courses is supported
if ( !function_exists( 'hotlock_exists_courses' ) ) {
	function hotlock_exists_courses() {
		return defined('TRX_ADDONS_CPT_COURSES_PT');
	}
}

// Return true if layouts is supported
if ( !function_exists( 'hotlock_exists_layouts' ) ) {
	function hotlock_exists_layouts() {
		return defined('TRX_ADDONS_CPT_LAYOUTS_PT');
	}
}

// Return true if dishes is supported
if ( !function_exists( 'hotlock_exists_dishes' ) ) {
	function hotlock_exists_dishes() {
		return defined('TRX_ADDONS_CPT_DISHES_PT');
	}
}

// Return true if it's team page
if ( !function_exists( 'hotlock_is_team_page' ) ) {
	function hotlock_is_team_page() {
		return defined('TRX_ADDONS_CPT_TEAM_PT')
					&& !is_search()
					&& (
...
if ( !function_exists( 'hotlock_is_services_page' ) ) {
	function hotlock_is_services_page() {
		return defined('TRX_ADDONS_CPT_SERVICES_PT')
					&& !is_search()
					&& (
...
if ( !function_exists( 'hotlock_is_portfolio_page' ) ) {
	function hotlock_is_portfolio_page() {
		return defined('TRX_ADDONS_CPT_PORTFOLIO_PT')
					&& !is_search()
					&& (
...
if ( !function_exists( 'hotlock_is_courses_page' ) ) {
	function hotlock_is_courses_page() {
		return defined('TRX_ADDONS_CPT_COURSES_PT')
					&& !is_search()
					&& (
...
if ( !function_exists( 'hotlock_is_dishes_page' ) ) {
	function hotlock_is_dishes_page() {
		return defined('TRX_ADDONS_CPT_DISHES_PT')
					&& !is_search()
					&& (
...
if ( !function_exists( 'hotlock_trx_addons_frontend_scripts' ) ) {
	function hotlock_trx_addons_frontend_scripts() {
		if (hotlock_is_on(hotlock_get_theme_option('debug_mode')) && file_exists(hotlock_get_file_dir('plugins/trx_addons/trx_addons.css'))) {
			wp_enqueue_style( 'hotlock-trx-addons',  hotlock_get_file_url('plugins/trx_addons/trx_addons.css'), array(), null );
			wp_enqueue_style( 'hotlock-trx-addons-editor',  hotlock_get_file_url('plugins/trx_addons/trx_addons.editor.css'), array(), null );
		}
		if (hotlock_is_on(hotlock_get_theme_option('debug_mode')) && file_exists(hotlock_get_file_dir('plugins/trx_addons/trx_addons.js')))
			wp_enqueue_script( 'hotlock-trx-addons', hotlock_get_file_url('plugins/trx_addons/trx_addons.js'), array('jquery'), null, true );
		// Load custom layouts from the theme if plugin not exists
		if ( hotlock_is_on(hotlock_get_theme_option('debug_mode')) ) {
			wp_enqueue_style( 'hotlock-layouts', hotlock_get_file_url('plugins/trx_addons/layouts/layouts.css') );
			wp_enqueue_style( 'hotlock-layouts-logo', hotlock_get_file_url('plugins/trx_addons/layouts/logo.css') );
			wp_enqueue_style( 'hotlock-layouts-menu', hotlock_get_file_url('plugins/trx_addons/layouts/menu.css') );
			wp_enqueue_style( 'hotlock-layouts-search', hotlock_get_file_url('plugins/trx_addons/layouts/search.css') );
			wp_enqueue_style( 'hotlock-layouts-title', hotlock_get_file_url('plugins/trx_addons/layouts/title.css') );
			wp_enqueue_style( 'hotlock-layouts-featured', hotlock_get_file_url('plugins/trx_addons/layouts/featured.css') );
		}
	}
}

// Merge custom styles
if ( !function_exists( 'hotlock_trx_addons_merge_styles' ) ) {
	function hotlock_trx_addons_merge_styles($list) {
...
if ( !function_exists( 'hotlock_trx_addons_merge_scripts' ) ) {
	function hotlock_trx_addons_merge_scripts($list) {
...
if ( !function_exists( 'hotlock_trx_addons_detect_blog_mode' ) ) {
	function hotlock_trx_addons_detect_blog_mode($mode='') {
		if ( hotlock_is_team_page() )
			$mode = 'team';
		else if ( hotlock_is_services_page() )
			$mode = 'services';
		else if ( hotlock_is_portfolio_page() )
			$mode = 'portfolio';
		else if ( hotlock_is_courses_page() )
			$mode = 'courses';
		else if ( hotlock_is_dishes_page() )
			$mode = 'dishes';
		return $mode;
	}
}
...
if ( !function_exists( 'hotlock_trx_addons_list_post_types' ) ) {
	function hotlock_trx_addons_list_post_types($list=array()) {
...
if ( !function_exists( 'hotlock_trx_addons_post_type_taxonomy' ) ) {
	function hotlock_trx_addons_post_type_taxonomy($tax='', $post_type='') {
...
if ( !function_exists( 'hotlock_trx_addons_get_post_categories' ) ) {
	function hotlock_trx_addons_get_post_categories($cats='') {

		if ( defined('TRX_ADDONS_CPT_TEAM_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_TEAM_PT) {
				$cats = hotlock_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_TEAM_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_SERVICES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_SERVICES_PT) {
				$cats = hotlock_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_SERVICES_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_PORTFOLIO_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_PORTFOLIO_PT) {
				$cats = hotlock_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_PORTFOLIO_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_COURSES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_COURSES_PT) {
				$cats = hotlock_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_COURSES_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_DISHES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_DISHES_PT) {
				$cats = hotlock_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_DISHES_TAXONOMY);
			}
		}
		return $cats;
	}
...
if ( !function_exists( 'hotlock_trx_addons_list_header_styles' ) ) {
	function hotlock_trx_addons_list_header_styles($list=array()) {
		if (hotlock_exists_layouts()) {
			$layouts = hotlock_get_list_posts(false, array(
...
if ( !function_exists( 'hotlock_trx_addons_list_footer_styles' ) ) {
	function hotlock_trx_addons_list_footer_styles($list=array()) {
		if (hotlock_exists_layouts()) {
			$layouts = hotlock_get_list_posts(false, array(
							'post_type' => TRX_ADDONS_CPT_LAYOUTS_PT,
							'meta_key' => 'trx_addons_layout_type',
...
if ( !function_exists( 'hotlock_trx_addons_get_post_date_wrap' ) ) {
	function hotlock_trx_addons_get_post_date_wrap($dt='') {
		return apply_filters('hotlock_filter_get_post_date', $dt);
	}
}

// Show date of the courses
if ( !function_exists( 'hotlock_trx_addons_get_post_date' ) ) {
	function hotlock_trx_addons_get_post_date($dt='') {
		...
		if ( defined('TRX_ADDONS_CPT_COURSES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_COURSES_PT) {
				...
				$dt = sprintf($dt < date('Y-m-d')
						? esc_html__('Started on %s', 'hotlock')
						: esc_html__('Starting %s', 'hotlock'),
...
if (!function_exists('hotlock_trx_addons_allow_override_options')) {
	function hotlock_trx_addons_allow_override_options($allow, $post_type) {
		return $allow
...
if (!function_exists('hotlock_trx_addons_allow_theme_icons')) {
	function hotlock_trx_addons_allow_theme_icons($allow, $post_type) {
		return $allow
...
if (!function_exists('hotlock_trx_addons_override_options_fields')) {
	function hotlock_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'hotlock'),
				"desc" => wp_kses_data( __("Team member's email", 'hotlock') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
...
if ( !function_exists( 'hotlock_trx_addons_editor_load_scripts_admin' ) ) {
	function hotlock_trx_addons_editor_load_scripts_admin() {
		// Add styles in the WP text editor
		add_editor_style( array(
							hotlock_get_file_url('plugins/trx_addons/trx_addons.editor.css')
							)
						 );
	}
}
...
if ( !function_exists( 'hotlock_trx_addons_sc_atts' ) ) {
	function hotlock_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'hotlock_trx_addons_sc_map' ) ) {
	function hotlock_trx_addons_sc_map($params, $sc) {
...
if ( !function_exists( 'hotlock_trx_addons_sc_output' ) ) {
	function hotlock_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_layouts_featured')))
			hotlock_sc_layouts_showed('featured', is_singular() && in_array(get_post_type(), array('post', 'page')) && has_post_thumbnail());
			...
			hotlock_sc_layouts_showed('cart', true);
		else if (in_array($sc, array('trx_sc_layouts_logo')))
			hotlock_sc_layouts_showed('logo', true);
		else if (in_array($sc, array('trx_sc_layouts_menu')))
			hotlock_sc_layouts_showed('menu', true);
		else if (in_array($sc, array('trx_sc_layouts_search')))
			hotlock_sc_layouts_showed('search', true);
...
if ( !function_exists( 'hotlock_trx_addons_slider_title' ) ) {
	function hotlock_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title']))
...
if (!function_exists('hotlock_sc_layouts_showed')) {
	function hotlock_sc_layouts_showed($name, $val=null) {
		if ($val!==null) {
			hotlock_storage_set_array('sc_layouts_components', $name, $val);
			return true;
		} else {
			return hotlock_storage_get_array('sc_layouts_components', $name) != '';
		}
	}
}

// Return image size multiplier
if (!function_exists('hotlock_get_retina_multiplier')) {
	function hotlock_get_retina_multiplier($force_retina=0) {
		static $mult = 0;
...
if (!function_exists('hotlock_build_slider_layout')) {
	function hotlock_build_slider_layout($args) {
...
if ( !function_exists( 'hotlock_trx_addons_featured_image' ) ) {
	function hotlock_trx_addons_featured_image($processed=false, $args=array()) {
		$args['show_no_image'] = true;
		$args['singular'] = false;
		$args['hover'] = hotlock_get_theme_option('image_hover');
		hotlock_show_post_featured($args);
		return true;
	}
}

// Return theme specific 'no-image' picture
if ( !function_exists( 'hotlock_trx_addons_no_image' ) ) {
	function hotlock_trx_addons_no_image($no_image='') {
		return hotlock_get_no_image($no_image);
	}
}

// Return theme-specific icons
if ( !function_exists( 'hotlock_trx_addons_get_list_icons' ) ) {
	function hotlock_trx_addons_get_list_icons($list, $prepend_inherit) {
		return hotlock_get_list_icons($prepend_inherit);
	}
}

// Return links to the social profiles
if (!function_exists('hotlock_get_socials_links')) {
	function hotlock_get_socials_links($style='icons') {
...
if (!function_exists('hotlock_get_share_links')) {
	function hotlock_get_share_links($args=array()) {
...
if (!function_exists('hotlock_show_share_links')) {
	function hotlock_show_share_links($args=array()) {
...
if (!function_exists('hotlock_get_category_image')) {
	function hotlock_get_category_image($term_id=0) {
...
if (!function_exists('hotlock_get_category_icon')) {
	function hotlock_get_category_icon($term_id=0) {
...
if (!function_exists('hotlock_get_post_counters')) {
	function hotlock_get_post_counters($counters='views') {
...
if (!function_exists('hotlock_get_list_animations_in')) {
	function hotlock_get_list_animations_in() {
...
if (!function_exists('hotlock_get_animation_classes')) {
	function hotlock_get_animation_classes($animation, $speed='normal', $loop='none') {
...
if (!function_exists('hotlock_get_comment_counters')) {
	function hotlock_get_comment_counters($counters = 'likes') {
...
if (!function_exists('hotlock_show_comment_counters')) {
	function hotlock_show_comment_counters($counters = 'likes') {
...
if (!function_exists('hotlock_trx_addons_query_sort_order')) {
	function hotlock_trx_addons_query_sort_order($q=array(), $orderby='date', $order='desc') {
		if ($orderby == 'views') {
...
if ( !function_exists( 'hotlock_trx_addons_theme_logo' ) ) {
	function hotlock_trx_addons_theme_logo($logo) {
		return hotlock_get_logo_image();
	}
}

// Return theme-specific post meta to the plugin
if ( !function_exists( 'hotlock_trx_addons_post_meta' ) ) {
	function hotlock_trx_addons_post_meta($meta, $args=array()) {
		return hotlock_show_post_meta($args);
	}
}

// Redirect action 'get_mobile_menu' to the plugin
// Return stored items as mobile menu
if ( !function_exists( 'hotlock_trx_addons_get_mobile_menu' ) ) {
	function hotlock_trx_addons_get_mobile_menu($menu) {
		return apply_filters('trx_addons_filter_get_mobile_menu', $menu);
	}
}

// Redirect action 'login' to the plugin
if (!function_exists('hotlock_trx_addons_action_login')) {
	function hotlock_trx_addons_action_login($link_text='', $link_title='') {
		do_action( 'trx_addons_action_login', $link_text, $link_title );
	}
}

// Redirect action 'search' to the plugin
if (!function_exists('hotlock_trx_addons_action_search')) {
	function hotlock_trx_addons_action_search($style, $class, $ajax) {
		do_action( 'trx_addons_action_search', $style, $class, $ajax );
	}
}

// Redirect action 'breadcrumbs' to the plugin
if (!function_exists('hotlock_trx_addons_action_breadcrumbs')) {
	function hotlock_trx_addons_action_breadcrumbs() {
		do_action( 'trx_addons_action_breadcrumbs' );
	}
}

// Redirect action 'show_layout' to the plugin
if (!function_exists('hotlock_trx_addons_action_show_layout')) {
	function hotlock_trx_addons_action_show_layout($layout_id='') {
		do_action( 'trx_addons_action_show_layout', $layout_id );
	}
}

// Redirect filter 'get_blog_title' to the plugin
if ( !function_exists( 'hotlock_trx_addons_get_blog_title' ) ) {
	function hotlock_trx_addons_get_blog_title($title='') {
		return apply_filters('trx_addons_filter_get_blog_title', $title);
	}
}

// Redirect filter 'prepare_css' to the plugin
if (!function_exists('hotlock_trx_addons_prepare_css')) {
	function hotlock_trx_addons_prepare_css($css='', $remove_spaces=true) {
		return apply_filters( 'trx_addons_filter_prepare_css', $css, $remove_spaces );
	}
}

// Redirect filter 'prepare_js' to the plugin
if (!function_exists('hotlock_trx_addons_prepare_js')) {
	function hotlock_trx_addons_prepare_js($js='', $remove_spaces=true) {
		return apply_filters( 'trx_addons_filter_prepare_js', $js, $remove_spaces );
	}
}

// Return text for the "I agree ..." checkbox
if ( ! function_exists( 'hotlock_trx_addons_privacy_text' ) ) {
    add_filter( 'trx_addons_filter_privacy_text', 'hotlock_trx_addons_privacy_text' );
    function hotlock_trx_addons_privacy_text( $text='' ) {
        return hotlock_get_privacy_text();
    }
}
...
if (!function_exists('hotlock_trx_addons_get_css')) {
	function hotlock_trx_addons_get_css($css, $colors, $fonts, $scheme='') {
...
.scheme_default .scheme_dark .sc_action_item .sc_action_item_link,
...
.sc_services_default .sc_services_item:hover .sc_button {
	background-color: {$colors['text_link']};
}
.sc_services_default .sc_services_item:hover .sc_services_item_title a{
	color: {$colors['text_link']};
}
.sc_services_default .sc_services_item:hover .sc_services_item_title a:hover{
	color: {$colors['bg_color']};
}
...
.sc_services_iconed .sc_services_item_header .sc_services_item_icon {
	color: {$colors['text_link']};
}
...
.sc_services_hover .sc_services_item_content .sc_services_item_subtitle a {
	color: {$colors['text_dark']};
}
...
.sc_team .sc_team_item_thumb .sc_team_item_socials .social_item a {
	...
	border-right-color: {$colors['alter_dark']};
}
...
.trx_addons_cv .trx_addons_scroll_to_top.show {
	color: {$colors['inverse_link']};
	border-color: {$colors['text_link']};
...
.trx_addons_cv .trx_addons_scroll_to_top.show:hover {
	color: {$colors['inverse_hover']};
