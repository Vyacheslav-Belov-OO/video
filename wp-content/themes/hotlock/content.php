...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'post_item_single post_type_'.esc_attr(get_post_type()) 
	...
	if (hotlock_is_on(hotlock_get_theme_option('seo_snippets'))) {
		?>
		<div class="structured_data_snippets">
			...
			<div itemprop="publisher" itemscope itemtype="https://schema.org/Organization">
				<div itemprop="logo" itemscope itemtype="https://schema.org/ImageObject">
					<?php 
					$hotlock_logo_image = hotlock_get_retina_multiplier(2) > 1 
										? hotlock_get_theme_option( 'logo_retina' )
										: hotlock_get_theme_option( 'logo' );
					if (!empty($hotlock_logo_image)) {
						$hotlock_attr = hotlock_getimagesize($hotlock_logo_image);
						?>
						<img itemprop="url" src="<?php echo esc_url($hotlock_logo_image); ?>">
	...
	if ( !hotlock_sc_layouts_showed('title') && !in_array(get_post_format(), array('link', 'aside', 'status', 'quote')) ) {
		...
		<div class="post_header entry-header">
			<?php
			// Post title
			the_title( '<h3 class="post_title entry-title"'.(hotlock_is_on(hotlock_get_theme_option('seo_snippets')) ? ' itemprop="headline"' : '').'>', '</h3>' );
			// Post meta
		...
		<?php
			...
			if ( is_single() && !is_attachment() ) {
				...
				<div class="post_meta post_meta_single"><?php
					...
					// Post taxonomies
					the_tags( '<span class="post_meta_item post_tags"><span class="post_meta_label">'.esc_html__('Tags:', 'hotlock').'</span> ', ', ', '</span>' );
